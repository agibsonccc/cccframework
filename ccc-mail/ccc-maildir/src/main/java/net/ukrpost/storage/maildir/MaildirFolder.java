...
public class MaildirFolder extends Folder implements UIDPlusFolder,
    ...
    private OutputStream getTmpFileOutputStream(File tmpFilename,
    	...
    	String path=tmpFilename.getAbsolutePath();
    	if(!(path.charAt(0)=='/'))
    		path="/" + path;
    	tmpFilename = new File(path);
    	if(!tmpFilename.exists())
    		FileMoverUtil.createFile(tmpFilename, false);
        ...
        try {
            for (int i = 0; i < msgs.length; i++) {
                ...
                try {
                    ...
                    if(!(tmptarget.getAbsolutePath().charAt(0)=='/')) {
                    	String path=tmptarget.getAbsolutePath();
                    	tmptarget = new File("/" + path);
                    }
